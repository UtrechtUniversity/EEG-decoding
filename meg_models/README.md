...
| channels | 'EEG' or ‘MEG’ if all channels should be used for classification. If you don’t want all channels, define a Nx1 cell-array with the selection of channels that should be included in classification, e.g. channels = {‘Iz’; ‘Oz’; ‘POz’} (used in PSR_loadTimelock).<br>(See ['http://www.fieldtriptoolbox.org/reference/ft_channelselection/'](the Fieldtrip Toolbox) for more input options) |
